...
class R3Adaptation(AdaptationInterface):
    ...
    def __init__(self, max_iterations: int = DEFAULT_R3_MAX_ITERATIONS, k: int = 1) -> None:
        super().__init__()
    ...
    def refine(self, loss_function: Callable, old_x: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.max_iterations):
            residual_function_values = loss_function(x).pow(self.k).abs().reshape(-1)
    ...
    def __str__(self) -> str:
        return (
            "r3"
            if self.max_iterations == DEFAULT_R3_MAX_ITERATIONS and self.k == 1
            else f"r3_{self.max_iterations}_{self.k}"
        )
