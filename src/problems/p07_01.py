...
class P0701Problem(ProblemInterface):
    def __init__(self):
        self.range = [0.0, 1.0]
    ...
    def exact_solution(self, x: torch.Tensor) -> torch.Tensor:
        return torch.pow((x+0.1), 0.7)

    def f_inner_loss(self, x: torch.Tensor, pinn: PINN) -> torch.Tensor:
        return dfdx(pinn, x, order=2) + 0.21 / torch.pow((x+0.1), 1.3)
    ...
    def compute_loss(self, x: torch.Tensor, pinn: PINN) -> torch.Tensor:
        ...
        boundary_loss_left = f(pinn, boundary_left) - 0.19952623149688796013524
        ...
        boundary_loss_right = dfdx(pinn, boundary_right, order=1) - 0.6802683013103936
