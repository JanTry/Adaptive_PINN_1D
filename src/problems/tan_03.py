...
class Tan03Problem(ProblemInterface):
    def __init__(self):
    ...
    def exact_solution(self, x: torch.Tensor) -> torch.Tensor:
        return torch.tan(x - 0.3)

    def f_inner_loss(self, x: torch.Tensor, pinn: PINN) -> torch.Tensor:
        return dfdx(pinn, x, order=2) + 2 * torch.sin(0.3 - x) / torch.pow(torch.cos(0.3 - x), 3)
    ...
    def compute_loss(self, x: torch.Tensor, pinn: PINN) -> torch.Tensor:
        ...
        boundary_loss_left = f(pinn, boundary_left) + 0.309336
        ...
        boundary_loss_right = dfdx(pinn, boundary_right, order=1) - 11.45053125149
