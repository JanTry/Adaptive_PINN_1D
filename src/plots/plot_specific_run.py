...
def plot_specific_run(run_id: int, problem_type: EProblems, adaptation: EAdaptations, tolerance: float=params.TOLERANCE,
                      learning_rate: float=params.LEARNING_RATE, layers: int=params.LAYERS, neurons: int=params.NEURONS,
                      epochs: int=params.NUMBER_EPOCHS, max_points: int=params.NUM_MAX_POINTS,
                      plot_training_points: bool=False):
    device='cpu'
    matplotlib.rcParams.update({'font.size': 15})

    path = os.path.join("results", problem_type.value, adaptation.value,
                        f'L{layers}_N{neurons}_'
                        f'P{max_points}_E{epochs}',
                        f'LR{learning_rate}_TOL{tolerance}', str(run_id))
    ...
    nn_approximator = torch.load(os.path.join(path, f'pinn.pt'))
    convergence_data = torch.load(os.path.join(path, f'convergence.pt'))
    n_iters = torch.load(os.path.join(path, f'n_iters.pt'))
    exec_time = torch.load(os.path.join(path, f'exec_time.pt'))
    ...
    if plot_training_points:
        point_data = torch.load(os.path.join(path, f'point_data.pt'))
    ...
    n_x = torch.linspace(x_range[0], x_range[1], steps=1000 + 2, requires_grad=True, device=device)[
          1:-1
          ].reshape(-1)
    ...
    fig.savefig(os.path.join(path, 'plots', f'pinn_solution'))
    plt.close(fig)
    ...
    fig.savefig(os.path.join(path, 'plots', f'exact_solution'))
    plt.close(fig)
    ...
    fig.savefig(os.path.join(path, 'plots', f'solutions'))
    plt.close(fig)
    ...
    ax.set_title(
        f"Error: NN_u - exact_solution,\n time = {exec_time:.2f}s, iterations = {n_iters}")
    fig.savefig(os.path.join(path, 'plots', f'error'))
    plt.close(fig)
    ...
    fig.savefig(os.path.join(path, 'plots', f'convergence'))
    plt.close(fig)
